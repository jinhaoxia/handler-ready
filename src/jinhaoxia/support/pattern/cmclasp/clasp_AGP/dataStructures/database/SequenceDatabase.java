package jinhaoxia.support.pattern.cmclasp.clasp_AGP.dataStructures.database;

import jinhaoxia.support.pattern.cmclasp.clasp_AGP.dataStructures.Item;
import jinhaoxia.support.pattern.cmclasp.clasp_AGP.dataStructures.Itemset;
import jinhaoxia.support.pattern.cmclasp.clasp_AGP.dataStructures.Sequence;
import jinhaoxia.support.pattern.cmclasp.clasp_AGP.dataStructures.creators.AbstractionCreator;
import jinhaoxia.support.pattern.cmclasp.clasp_AGP.dataStructures.creators.ItemAbstractionPairCreator;
import jinhaoxia.support.pattern.cmclasp.clasp_AGP.idlists.IDList;
import jinhaoxia.support.pattern.cmclasp.clasp_AGP.idlists.creators.IdListCreator;
import jinhaoxia.support.pattern.cmclasp.clasp_AGP.tries.Trie;
import jinhaoxia.support.pattern.cmclasp.clasp_AGP.tries.TrieNode;
...
public class SequenceDatabase {
    ...
    public double loadDatabase(List<int[]> database, double minSupport) {
        for (int[] tran : database) {
            addTransaction(tran);
        }
    ...
    public void addTransaction(int[] tran) {
        ItemAbstractionPairCreator pairCreator = ItemAbstractionPairCreator.getInstance();
        long timestamp = -1;
        Sequence sequence = new Sequence(sequences.size());
        Itemset itemset = new Itemset();
        ...
        for (int i = 0; i < tran.length; i++) {
                // extract the value for an item
                Item item = itemFactory.getItem(tran[i]);
                TrieNode node = frequentItems.get(item);
                if (node == null) {
                    IDList idlist = idListCreator.create();
                    node = new TrieNode(pairCreator.getItemAbstractionPair(item, abstractionCreator.createDefaultAbstraction()), new Trie(null, idlist));
